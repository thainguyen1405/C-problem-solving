...
int findFirstValidPermutation(int check[], int l[], int r);
...
int main() 
{
	...
	//Recall function
    findFirstValidPermutation(check, l, 0);
...
// Recursive function to find the first valid permutation
int findFirstValidPermutation(int check[], int l[], int r) 
{
    if (r == n) 
	{
    ...
    for (int i = 0; i < n; i++) 
	{
        if (!l[i]) 
		{
            check[r] = i;
            l[i] = 1;  
            if (findFirstValidPermutation(check, l, r + 1)) 
			{
                return 1;  
            }
            l[i] = 0;  
